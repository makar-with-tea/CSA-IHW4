...
void* f_customer(void *args){
    ...
    pthread_barrier_wait(&barrier); // показываем, что данный покупатель покинул магазин
    return nullptr;
}

void* f_department(void *args){
    int dep = *(int*)args; // номер отдела
    while (!finished) { // цикл работает, пока не все покупатели завершили покупки
...
int main(int argc, char* argv[]) {
    ...
    pthread_barrier_init(&barrier, nullptr, n + 1); // требуется (n+1) тредов, так как учитывается основной тред, а также все треды покупателей
